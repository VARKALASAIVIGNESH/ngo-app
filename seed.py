...
def seed_database():
    ...
    with app.app_context():
        ...
        db.drop_all()
        db.create_all()
        print("[+] Recreated all database tables in SQLite.")
        ...
        db.session.commit()
        print(f"[+] Created 3 users: 1 Admin ('srinivas'), 2 Teachers ('teacher', 'david.kumar').")

        # 3. Seed Students (10 diverse student records)
        students_data = [
            {
        ...
        db.session.commit()
        print(f"[+] Created {len(students)} student beneficiaries with IDs (NGO-2026-0001 to NGO-2026-0010).")
        ...
        db.session.commit()
        print(f"[+] Created {len(sessions)} sessions (4 completed, 2 scheduled).")

        # 5. Seed Attendance for completed sessions
        ...
        for i, sess in enumerate(completed_sessions):
            for j, student in enumerate(active_students):
                # Deterministic variety for rich demo data
        ...
        db.session.commit()
        print(f"[+] Created {att_count} realistic attendance records.")

        # 6. Seed Weekly Feedbacks
        feedbacks_data = [
            {
                'student_id': students[0].id,  # Aarav
                'teacher_id': teacher1.id,
            ...
            {
                'student_id': students[1].id,  # Ananya
                'teacher_id': teacher2.id,
            ...
            {
                'student_id': students[2].id,  # Rohan
                'teacher_id': teacher1.id,
                ...
                'comments': 'Recommend paired tutoring with Aarav or extra practice sheets.'
            },
            {
                'student_id': students[2].id,  # Rohan
                'teacher_id': teacher1.id,
            ...
            {
                'student_id': students[3].id,  # Priya
                'teacher_id': teacher2.id,
            ...
            {
                'student_id': students[4].id,  # Bilal
                'teacher_id': teacher1.id,
            ...
            {
                'student_id': students[5].id,  # Fatima
                'teacher_id': teacher2.id,
        ...
        db.session.commit()
        print(f"[+] Created {len(feedbacks_data)} weekly feedback and progress evaluation records.")
